...
| `xcstrings_check_coverage` | Check key language coverage with explicit measured or not-applicable state |
| `xcstrings_stats_coverage` | Get overall coverage statistics, including compact tri-state summaries |
| `xcstrings_stats_progress` | Get translation progress by language |
| `xcstrings_batch_stats_coverage` | Get coverage for multiple files at once, including compact tri-state summaries |
...
xcstrings-crud stats coverage --file path/to/Localizable.xcstrings --compact
...
xcstrings-crud stats batch-coverage -f file1.xcstrings file2.xcstrings file3.xcstrings --compact
```
...
- `get key` and `xcstrings_get_key` return `KeyInfo` metadata together with translations. The JSON includes `comment`, `isCommentAutoGenerated`, `shouldTranslate`, `extractionState`, `languages`, and `translations`.
...
- `check key` and `xcstrings_check_key` become locale-strict when a language is provided: they return `true` only when that locale has an actual localization record, even for keys marked `shouldTranslate: false`.
